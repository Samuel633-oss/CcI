// Copyright (c) 2025-present The Alara Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries alara-node, alara-gui.
  -M, --monolithic       Run monolithic binaries alarad, alara-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'alara-qt [ARGS]' or 'alara-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'alarad [ARGS]' or 'alara-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'alara-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'alara-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'alara-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_alara [ARGS]'.
  chainstate [ARGS] Run alara kernel chainstate util, equivalent to running 'alara-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_alara [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_alara-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "alara-gui" : "alara-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "alara-node" : "alarad");
        } else if (cmd.command == "rpc") {
            args.emplace_back("alara-cli");
            ...
            args.emplace_back("alara-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("alara-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_alara");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("alara-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_alara");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_alara-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("alara-util");
        } else {
...
//! Execute the specified alarad, alara-qt or other command line in `args`
...
//!             be a relative path to a alarad, alara-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/alara ...
